...
function renderPartControls(parts) {
    ...
    parts.forEach((part, index) => {
        ...
        let defaultChoice = 'soprano-recorder';
        if (parts.length === 2) {
            if (index === 1) defaultChoice = 'alto-recorder';
        } else if (parts.length === 3) {
            if (index === 1) defaultChoice = 'alto-recorder';
            if (index === 2) defaultChoice = 'bass-recorder';
        } else if (parts.length >= 4) {
            if (index === 1) defaultChoice = 'alto-recorder';
            if (index === 2) defaultChoice = 'tenor-recorder';
            if (index === 3) defaultChoice = 'bass-recorder'; // Typical SATB!
        }
        
        select.innerHTML = `
            <optgroup label="Recorder Family">
                <option value="bass-recorder" ${defaultChoice==='bass-recorder'?'selected':''}>Bass Recorder</option>
                <option value="tenor-recorder" ${defaultChoice==='tenor-recorder'?'selected':''}>Tenor Recorder</option>
                <option value="alto-recorder" ${defaultChoice==='alto-recorder'?'selected':''}>Alto Recorder</option>
                <option value="soprano-recorder" ${defaultChoice==='soprano-recorder'?'selected':''}>Soprano Recorder</option>
            </optgroup>
            <optgroup label="Keyboards">
                <option value="acoustic_grand_piano" ${defaultChoice==='acoustic_grand_piano'?'selected':''}>Piano</option>
                <option value="harpsichord" ${defaultChoice==='harpsichord'?'selected':''}>Harpsichord</option>
            </optgroup>
            <optgroup label="Strings">
                <option value="cello" ${defaultChoice==='cello'?'selected':''}>Cello</option>
            </optgroup>
        `;
