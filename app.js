            ...
            try {
                ...
                if (!xmlContent) {
                    for (const relativePath in zip.files) {
                        if (relativePath.endsWith('.xml') && !relativePath.startsWith('META-INF/')) {
...
async function prepareAudio() {
    ...
    selects.forEach(select => {
        ...
        if (!sfInstruments[id]) {
            if (sfName === 'recorder') {
                sfInstruments[id] = new CustomRecorderSampler(ac, { destination: masterGain, type: val });
            } else {
...
function triggerMasterPlayback() {
    ...
    currentNotes.forEach(note => {
        ...
        Tone.Transport.schedule((time) => {
            ...
            if (chk && chk.checked) return; // Skip excluded parts
