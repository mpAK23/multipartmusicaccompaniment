...
class CustomRecorderSampler {
    ...
    start({ note, time, duration }) {
        ...
        else if (this.type === 'alto-recorder') shiftedNote += 0;
        else if (this.type === 'tenor-recorder') shiftedNote += 0;
        else if (this.type === 'bass-recorder') shiftedNote += 0;
...
function handleFile(file) {
    ...
    if (!name.endsWith('.xml') && !name.endsWith('.musicxml') && !name.endsWith('.mxl') && !name.endsWith('.accomp')) {
        updateStatus('Please upload a valid .xml, .musicxml, .mxl, or .accomp file.', true);
        return;
    }
    
    if (name.endsWith('.accomp')) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                updateStatus('Share file loaded successfully!', false);
            } catch (err) {
                console.error(err);
                updateStatus('Error loading .accomp file.', true);
            }
        };
...
function parseXmlString(xmlString) {
    ...
    try {
        ...
        if (currentNotes.length > 0) {
            ...
            document.getElementById('download-share-btn').disabled = false;
            ...
            document.getElementById('download-share-btn').disabled = true;
...
function triggerMasterPlayback() {
    ...
    if (loopChk) {
        let isLooping = isRecording ? false : loopChk.checked;
...
document.getElementById('download-share-btn').addEventListener('click', async () => {
    ...
    updateStatus('Generating share file...', false);
    const zip = new JSZip();
    ...
    const a = document.createElement('a');
    a.href = url;
    let safeName = fileNameDisplay.textContent.replace('.musicxml', '').replace('.xml', '').replace('.mxl', '');
    if (safeName === "No file selected") safeName = "SharedSong";
    a.download = `${safeName}.accomp`;
    a.click();
    URL.revokeObjectURL(url);
    updateStatus('Share file downloaded successfully!', false);
});
